...
# =====================================================================================
# --- ADMIN: DATABASE STATS & CLEANUP ---
# =====================================================================================

@bot.on_message(filters.command("dbstats") & filters.private & filters.user(Config.OWNER_ID))
async def db_stats_command(client: Client, message: Message):
    stats = await db.get_stats()
    if stats is None:
        await message.reply_text("⚠️ Database connected nei (DATABASE_URL set kora nei).", quote=True)
        return

    used_mb = stats["storage_size"] / (1024 * 1024)
    limit_mb = Config.MONGO_FREE_LIMIT_MB
    free_mb = max(limit_mb - used_mb, 0)
    percent_used = (used_mb / limit_mb) * 100 if limit_mb else 0

    text = (
        "📊 **Database Stats**\n\n"
        f"🔗 Total Links: `{stats['count']}`\n"
        f"💾 Used Storage: `{get_readable_file_size(stats['storage_size'])}`\n"
        f"📦 Free Space: `{free_mb:.2f} MB` (of {limit_mb} MB limit)\n"
        f"📈 Usage: `{percent_used:.1f}%`\n"
        f"🗂️ Index Size: `{get_readable_file_size(stats['index_size'])}`"
    )
    await message.reply_text(text, quote=True)

@bot.on_message(filters.command("cleandb") & filters.private & filters.user(Config.OWNER_ID))
async def clean_db_command(client: Client, message: Message):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Yes, Delete All", callback_data="cleandb_confirm")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cleandb_cancel")]
    ])
    await message.reply_text(
        "⚠️ **Are You Sure?**\n\n"
        "Eita **shob** saved links database theke permanently delete kore debe "
        "(pura history clean, storage full khali hoye jabe). Ei action **undo kora jabe na**.",
        reply_markup=keyboard, quote=True
    )

@bot.on_callback_query(filters.regex("^cleandb_"))
async def clean_db_callback(client: Client, callback_query: CallbackQuery):
    if callback_query.from_user.id != Config.OWNER_ID:
        await callback_query.answer("⛔ Only owner can use this.", show_alert=True)
        return

    if callback_query.data == "cleandb_confirm":
        deleted = await db.clean_all()
        await callback_query.message.edit_text(
            f"✅ **Database Clean Hoye Geche!**\n\n🗑️ `{deleted}` links delete kora hoyeche."
        )
    else:
        await callback_query.message.edit_text("❌ Cancelled. Kichu delete kora hoyni.")
